# Track Manifest — You Probably Think This Song Is About You Too

## Side A
| # | Title | Status | Themes | Connects To |
|---|-------|--------|--------|-------------|
| 000 | The Groove | Locked | silence, the beginning before the beginning, the needle, the chair | Vol 1: 001 |
| Sidetrack | (interlude) | Locked | distraction as arrival, the side of the groove | Track 000 |
| 001 | The Commute | Demo | the drive, observation, screen vs. windshield, spring, the pull toward something unnamed | Vol 1: 001, 006, Track 000 |
| 002 | The Diner | Demo | simultaneity, Pepe, Ireland, the Dark Room, the Rescue, the Out-of-body transition | Vol 1: 018, 021-024, Track 001 |

## The Flip
*Unmarked. Somewhere in the tracklist. You won't know until it's there.*

## Side B
| # | Title | Status | Themes | Connects To |
|---|-------|--------|--------|-------------|
| 003 | The Landing | Demo | re-entry, the collision, the body you land in, the slow-motion second, everyone and no one, THE FLIP | Track 002, Vol 1: 001, 003, 004 |
| 004 | Substrate Firing | Locked | inhabitation (the solitary man), the fullness of needing nothing, the stutter, Moby Dick, the car as omen, pack-pack-pack | Track 002, 003, Vol 1: 001 |
| 005 | The Wrong Reflection | Demo | inhabitation (the mirror man), identity as borrowed clothes, the proposal, running toward, love story hiding under noir | Track 004, 003, 002, Vol 1: 014 |
| 006 | One Second | Demo | the proposal, the diner cast assembling, simultaneity from inside, inhabitation complete, one second as eternity | Track 005, 002, Vol 1: 001 |
| 007 | Lipstick and Red Dress | Demo | inhabitation (the woman), the red dress as evidence, sneaking as ritual, guilt, diner from inside the betrayal, Pepe's wife | Track 006, 005, 002, Vol 1: 018 |
| 008 | The Asking | Demo | violent ejection, inhabitation (the beggar), hunger as identity, invisibility, alley family, the headlights, none of it happened | Track 007, 006, 002, Vol 1: 004 |
| 009 | The Tip | Demo | inhabitation (the waitress), the mundane as identity, survival arithmetic, the argument, headlights unnoticed, invisible grind | Track 008, 006, 002, Vol 1: 004 |
| 010 | The Entrance | Demo | inhabitation (the loud man), the force of presence, ex-teammates, locker room tribe, being remembered, sacred vineyard | Track 009, 002, Vol 1: 008 |
| 011 | The Shift | Demo | inhabitation (the waiter), the morning checklist, the hangover, mop closet, congratulations to erasure, step over/around | Track 010, 002, Vol 1: 004 |
| 012 | The Pour | Demo | inhabitation (the bartender), Bari origins, mother's limoncello, bottle flip show, professional distance, Tokyo silence | Track 011, 002, Vol 1: 018 |
| 013 | The Third Thing | Demo | dream layer, CVS reference, stapler within stapler, narrator writing while driving, meta-commentary, love all the way down | Track 012, 002, Vol 1: 025 |
| 014 | The Twist | Demo | investigator investigated, the detective as the eighth figure, Pepe's client, the camera, the case file, the twist reveal | Track 013, 002, Vol 1: 014 |

## Bonus Track
| # | Title | Status | Themes | Connects To |
|---|-------|--------|--------|-------------|
| -0321 | Dr. Seen Returns | Locked | the second examination, substrate empathy, the dispatcher, the therapy frame revealed, the brain was never just one brain | Vol 1: 018, ALL Vol 2 tracks |

## The Hidden Track
*Not listed here. That's the point.*

## Bonus Material (Archived Drafts)
- **Track 006 — The B-Side**: Quantum Leap seam, bar with no name, man with the knob, the kid, wrong case, the hum (saved in `tracks/bonus-material/`)
- **Track 008 — The Collision**: Layer B characters meeting, the waitress, the clerk, the man in the gray coat, the ledger writer (saved in `tracks/bonus-material/`)
